...
func (loader *AssemblyLoader) Load(assemblyName string) (*Assembly, error) {
	...
	// already loaded
	if assembly, alreadyLoaded = loader.loadedAssemblies[assemblyName]; !alreadyLoaded {
		assemblyFilepath := path.Join(loader.basePath, assemblyName+".dll")
		if assembly, err = loader.loadAssemblyFile(assemblyFilepath); err != nil {
			return nil, err
		}

		loader.loadedAssemblies[assemblyName] = assembly
	}
